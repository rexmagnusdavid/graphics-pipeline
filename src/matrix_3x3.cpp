...
auto Matrix3x3::operator[](int index) -> Vector3& {
...
auto Matrix3x3::operator*(Vector3 right) -> Vector3 {
	...
	ret[0] = matrix[0].Dot(right);
	ret[1] = matrix[1].Dot(right);
	ret[2] = matrix[2].Dot(right);
...
auto Matrix3x3::operator*(Matrix3x3 right) -> Matrix3x3 {
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			ret[i][j] = matrix[i].Dot(right.GetColumn(j));
		}
	}
...
auto Matrix3x3::GetColumn(int index) -> Vector3 {
	...
	ret[0] = matrix[0][index];
	ret[1] = matrix[1][index];
	ret[2] = matrix[2][index];
...
auto Matrix3x3::GetTranspose() -> Matrix3x3 {
	...
	ret[0] = matrix.GetColumn(0);
	ret[1] = matrix.GetColumn(1);
	ret[2] = matrix.GetColumn(2);
...
auto Matrix3x3::GetInverse() -> Matrix3x3 {
    ...
    Vector3 column_0 = matrix.GetColumn(0);
    Vector3 column_1 = matrix.GetColumn(1);
    Vector3 column_2 = matrix.GetColumn(2);
